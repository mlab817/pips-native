...
import {Box, Center, Flex, Heading, HStack, Icon, ScrollView, Text, VStack} from "native-base";
...
const notifications = [
	{
		...
		"data": {
			...
			"message": "Your account is using an insecure password. Please change your password ASAP."
		},
...
export default function NotificationScreen() {
	return (
		<Box flex={1} safeArea>
			<Heading py={3} px={2}>Notifications</Heading>
					...
					renderItem={({ item }) => (
						<Box
							...
							<HStack space={2}>
								...
								<VStack w='80%'>
									<Text fontWeight='thin' fontSize={12} color={Colors.black}>{item.data?.subject}</Text>
									<Text isTruncated fontSize={12} noOfLines={3}>{item.data?.message}</Text>
									<Text fontSize={10}>{moment(item.created_at).format('MM/DD/YY HH:MM')}</Text>
								</VStack>
							</HStack>
						</Box>
